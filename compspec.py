...
AE= F/(4.*np.pi*Rrs*(rhos*vpr*vps**5)**.5)
...
AS = M0*((2.*np.pi*freq)**n)/(1.+(2.*np.pi*freq/(2.*np.pi*f0))**gamma)
...
plt.semilogx(1./freq, Aerr, label='Resp Error Brune Spectra Mw=' + str(Mw) + ' at ' + str(Rrs/1000.) + ' km' )
plt.semilogx(1./freq, AerrSTS1, label='Resp Error Brune Spectra Mw=' + str(Mw) + ' at ' + str(Rrs/1000.) + ' km' )
plt.legend()
plt.xlabel('Period (s)')
plt.ylim((-200., -100.))
plt.ylabel('Power (dB rel. 1 $(m/s^2)^2/Hz)$')
plt.xlim((2.*min(1./freq),10.))
plt.subplot(2,1,2)
plt.semilogx(1./freq, A -Aerr,label='Error No Accelerometer Response')
plt.semilogx(1./freq, A -AerrSTS1,label='Error No STS-1')
plt.legend()
plt.ylabel('Error (dB)')
plt.xlabel('Period (s)')
plt.xlim((2.*min(1./freq),10.))
